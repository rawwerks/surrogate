...
**Alias and function preservation is an invariant.** Wrapping a command must not destroy the user's alias or function for that name. The generated snippet's top-level preamble must, for every wrapped command, detect whether the name resolves to a shell function, a shell alias, or nothing, and capture the user's original meaning before installing the wrapper. Shell functions must be renamed to `_SURR_ORIG_<cmd>_fn` in-process and left unexported. Shell aliases must be captured into `_SURR_ALIAS_<cmd>`. A `_SURR_KIND_<cmd>` sentinel records which case applies (`function`, `alias`, or `bare`).
...
- **Function-backed**: the wrapper calls the renamed function `_SURR_ORIG_<cmd>_fn` directly in-process and does **not** zmx-wrap. Wrapping a shell function would require exposing its body across a process boundary — either via a bash function-export (which bash serializes into the environment as `BASH_FUNC_<name>%%=()  { ... }` and makes visible to every child process) or by writing the body to a temp file for a child shell to source. Shell functions can contain secrets, private paths, or credentials, so either mechanism is a real private-data leakage primitive. The safe default is to preserve the function's behavior exactly — it still runs, it just runs in the parent shell rather than a new zmx session. Users who want a command wrapped in zmx should bind it to an alias or a real binary instead of a shell function.
