...
The managed block must be installed at the **end** of the rc file, not the beginning. In `commands` mode, the rc file may define aliases earlier (for example `alias codex='bunx ...'`) that would otherwise shadow our command wrappers — bash aliases win over shell functions regardless of definition order. To defeat that, the generated snippet must emit `unalias <cmd>` as **top-level** statements before the managed `if [[ $- == *i* ]]` block, for every wrapped command. Doing it as top-level statements (rather than inside the block) is load-bearing: bash parses a compound command as a unit and alias-expands during the parse, so a runtime `unalias` inside the block fires too late and the function definitions inside the block fail with a syntax error when one of the wrapped command names is still aliased. Fish does not need the equivalent because fish implements `alias` as a function, and redefining the function with our wrapper wins when our snippet runs last.
